...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/nishadsinghi/Desktop/Experiment/audio_lastrun.py',
...
# Initialize components for Routine "silence"
silenceClock = core.Clock()
...
sound_2 = sound.Sound('Beep.wav', secs=1.0, stereo=True)
...
trials = data.TrialHandler(nReps=5, method='random', 
    ...
    trialList=data.importConditions('audio.xlsx'),
...
for thisTrial in trials:
    ...
    # update component parameters for each repeat
    sound_1.setSound(Direction, secs=3)
    sound_1.setVolume(10, log=False)
    # keep track of which components have finished
    ...
    # ------Prepare to start Routine "silence"-------
    t = 0
    silenceClock.reset()  # clock
    frameN = -1
    continueRoutine = True
    routineTimer.add(3.000000)
    # update component parameters for each repeat
    # keep track of which components have finished
    silenceComponents = [text_2]
    for thisComponent in silenceComponents:
        if hasattr(thisComponent, 'status'):
            thisComponent.status = NOT_STARTED
    
    # -------Start Routine "silence"-------
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = silenceClock.getTime()
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    continueRoutine = True
    # update component parameters for each repeat
    sound_2.setSound('Beep.wav', secs=1.0)
    sound_2.setVolume(10, log=False)
    import random
    time = random.random()*0.5 + 2
    # keep track of which components have finished
    beepComponents = [sound_2, text_3]
